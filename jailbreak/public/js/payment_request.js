...
frappe.ui.form.on('Payment Request', {
	refresh: function (frm) {
		// Check if payment request mark paid capability is enabled
		jailbreak.check_capability("payment_request_mark_paid").then((enabled) => {
			if (enabled) {
		...
		// Check if payment request reinitiate charge capability is enabled
		jailbreak.check_capability("payment_request_reinitiate_charge").then((enabled) => {
			if (enabled && 
